...
class BaseModelTrainer:
    def __init__(self, model_type: str, stats):
    ...
    def save(self, model, filepath: str, num_rows: int, train_indices: dict = None) -> None:
        ...
        if train_indices:
            indices_path = filepath.replace(".pkl", "_indices.json")
    ...
    def run(self, X: np.ndarray, model_path: str, train_indices: dict = None) -> None:
        ...
        """
        model = self.train(X)
        model_file = f"models/{self.model_type}/{model_path}.pkl"
        self.save(model, model_file, len(X), train_indices)
