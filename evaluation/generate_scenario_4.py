...
def generate_counter_trade(total_value):
    ...
    val = round(random.randint(total_value - 300, total_value + 300) / 100) * 100
    return [(-val, -val)]

def generate_zero_positions(n):
...
def write_to_csv(data, filename="evaluation/data/generated_scen_4.csv"):
